...
# Load preprocessors separately
@st.cache_resource
def load_preprocessors():
    return joblib.load("preprocessing/preprocessor.pkl.gz"), joblib.load("preprocessing/le_target.pkl.gz")

preprocessor, le_target = load_preprocessors()
...
# Target column name (Telco Churn)
target_col = "Churn"
...
if uploaded_file is not None:
    ...
    if target_col not in test_df.columns:
        ...
        # ✅ FIXED: Manual preprocessing BEFORE prediction
        if st.button("Evaluate Model"):
            # Step 1: Prepare features and target
            X_test = test_df.drop(columns=[target_col])
            y_test_raw = test_df[target_col]
            
            # Step 2: MANUAL PREPROCESSING (THIS WAS MISSING!)
            X_test_proc = preprocessor.transform(X_test)
            y_test = le_target.transform(y_test_raw)
            
            # Step 3: Predict
            y_pred = selected_model.predict(X_test_proc)
            ...
            if hasattr(selected_model, 'predict_proba'):
                y_proba = selected_model.predict_proba(X_test_proc)
